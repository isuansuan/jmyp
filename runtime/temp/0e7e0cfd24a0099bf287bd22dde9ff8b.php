<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:68:"C:\AppServ\www\jmyp\public/../application/admin\view\shop\index.html";i:1523276755;s:54:"C:\AppServ\www\jmyp\application\admin\view\layout.html";i:1524573331;}*/ ?>
...
<div class="panel panel-default">
	<div class="panel-heading">
		...
		<a class="btn btn-primary" href="<?php echo url('add'); ?>"><span class="glyphicon glyphicon-plus"></span>添加商品</a>
		<div class="pull-right" style="margin-top:5px;margin-left: 25px;">共有<b id="tot" class='sj'><?php echo $tot; ?></b>条数据</div>
	...
	<div class="panel-body">
		<table class="table userIndexTable table-bordered">
			...
			<tr id="tr<?php echo $dat['id']; ?>">
				<th><input type="checkbox" name="id" class="check" value="<?php echo $dat['id']; ?>"><?php echo $dat['id']; ?> </th>
				<th><?php echo mb_substr($dat['sname'],0,8,'utf-8'); ?>..</th>
				<th><img src="/static/upload/shop/<?php echo $dat['simg']; ?>" alt="" height="50px" width="120px"></th>
				<th><?php echo $dat['price']; ?></th>
				<th><?php echo $dat['stock']; ?></th>
				<th>
					<?php if($dat['shelf'] == '0'): ?>
					<a href="javascript:;" class="btn btn-danger" onclick="shelf(this,<?php echo $dat['id']; ?>,<?php echo $dat['shelf']; ?>)">下架</a>
					<?php else: ?>
					<a href="javascript:;" class="btn btn-info" onclick="shelf(this,<?php echo $dat['id']; ?>,<?php echo $dat['shelf']; ?>)">上架</a>
					<?php endif; ?>
					
				</th>
				<th><?php echo $dat['sales']; ?></th>
				<th><?php echo $dat['visit']; ?></th>
                <td><?php echo $dat['visittime']==0?'从未访问':date("Y-m-d H:i:s",$dat['visittime']); ?></td>
				<th><?php echo mb_substr($dat['contact'],0,5,'utf-8'); ?>.....</th>
				<th><a href="<?php echo url('detail',array('id'=>$dat['id'])); ?>">点击查看详情</a></th>
				<th><?php echo $dat['bname']; ?></th>
				<th><?php echo $dat['cname']; ?></th>
...
function shelf(obj,id,val){
	if(val){
		$.post("<?php echo url('ajax_shelf'); ?>",{id:id,shelf:0},function(data){
			if(data){
				$(obj).html('下架');
		...
		$.post("<?php echo url('ajax_shelf'); ?>",{id:id,shelf:1},function(data){
			if(data){
				$(obj).html('上架');
